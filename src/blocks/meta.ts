...
const meta: Record<BlockType, BlockMetaEntry> = {
  ...
  // ── Discrete ──
  [BlockType.UnitDelay]: { type: BlockType.UnitDelay, category: 'Discrete', math: 'z^{-1}', doc: 'Unit delay (one sample)', paramSpec: { initialValue: { type: 'number', default: 0, label: 'Initial Value' } } },
  [BlockType.DiscreteIntegrator]: { type: BlockType.DiscreteIntegrator, category: 'Discrete', math: '\\frac{1}{1-z^{-1}}', doc: 'Discrete-time integrator', paramSpec: { method: { type: 'select', default: 'forward-euler', label: 'Integration Method' }, initialValue: { type: 'number', default: 0, label: 'Initial Value' } } },
  [BlockType.DiscreteTransferFcn]: { type: BlockType.DiscreteTransferFcn, category: 'Discrete', math: '\\frac{N(z)}{D(z)}', doc: 'Discrete transfer function', paramSpec: { num: { type: 'array', default: [1], label: 'Numerator (descending z)' }, den: { type: 'array', default: [1, -0.5], label: 'Denominator (descending z)' } } },
  [BlockType.Memory]: { type: BlockType.Memory, category: 'Discrete', math: 'M', doc: 'Hold previous input value', paramSpec: { initialValue: { type: 'number', default: 0, label: 'Initial Value' } } },
