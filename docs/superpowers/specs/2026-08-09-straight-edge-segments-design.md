...
- **Empty array (`[]`)** = auto-routing. The renderer computes a default orthogonal path:
  - Ports share the same y → single straight horizontal line (source port is on the right, target port on the left).
...
- **Non-empty** = user-fixed routing. The polyline passes through waypoints in order. Once a user drags a segment, waypoints materialize and the routing freezes (no longer follows node movement).
...
**Vertex expansion:** the rendered polyline vertices are `expandPoints(points, sourcePos, targetPos)` — `[source, ...waypoints, target]` plus any implicit midpoint-jog vertices when waypoints are empty. Path building, hit testing, and segment translation all operate on the expanded vertex list, never on the raw `points` array directly.
...
```
expandPoints(points: XYPosition[], sourcePos: Position, targetPos: Position): XYPosition[]
  - points = [source, ...waypoints, target]
  - Returns the full rendered vertex list.
  - waypoints non-empty → just points (each consecutive pair is already axis-aligned).
  - waypoints empty, same y → [source, target].
  - waypoints empty, different y → [source, midpointJog, target] where midpointJog = ((sx+tx)/2, sy) —
...
1. `onPointerDown` on a segment:
   ...
   - **Materialize:** if the edge currently has empty waypoints (auto-routed), convert `V` (minus source/target) into stored waypoints now. This freezes routing the moment the user touches a line.
   - Determine the drag mode:
     - If the segment is bounded by a vertex on at least one end → **segment translation mode**. Record the segment's perpendicular axis and the starting vertex positions.
     - If the segment has no interior vertex on either end (straight auto-routed segment) → **bend creation mode**. Insert a waypoint at the cursor (orthogonalized), then immediately enter translation mode on the newly created bend.
...
4. `onDoubleClick` on a waypoint marker: `removeWaypoint` + update store.
5. Click on edge (no drag) → select edge (React Flow handles selection via `edgesFocusable`; verify the hit path forwards click events).
...
- `onEdgesChange` already wired to `applyEdgeChanges` — handles selection changes. Verify it doesn't clobber `waypoints` on select (it merges changes into existing edge objects, so data is preserved).
